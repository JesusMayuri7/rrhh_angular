...
@Component({
  selector: 'zd-reporte-ejecucion',  
  templateUrl: './reporte_ejecucion.component.html',
  styleUrls: ['./reporte_ejecucion.component.css'],
  encapsulation: ViewEncapsulation.None,
  providers:[ReportePlanillaService,ExcelService,MessageService,DialogService],

})
export class ReporteEjecucionComponent implements OnInit,OnDestroy {
  vigencias:any[];
  meses:any[];
  mes:number=0;
 ...
 /*   {
    ...
    store: new CustomStore({
      load: function(loadOptions) {        
        return a.httpClient.get("http://rrhh.pvn.gob.pe/api/presupuestal/ejecucion",
        {headers :header})
        .toPromise()
        .then(result => {              
          return {          
