...
@Component({
  selector: 'zd-proyeccion-dos-cap',  
  templateUrl: './proyeccion_dos_cap.component.html',
  styleUrls: ['./proyeccion_dos_cap.component.css'],
  encapsulation: ViewEncapsulation.None,
  providers:[PlanillaCapService,ExcelService,MessageService,DialogService],
})
export class ProyeccionDosCapComponent implements OnInit,OnDestroy {
  fuentes:any[];
  dataSource: any = {};
  store: CustomStore;
 ...
 ngOnDestroy() {
  ...
  ngOnInit() {      
    this.vigencias= [
      ...
      { label: 'TODOS', value: 'SOSTENIBLE,31/12/2020'},
  ...
  onClickNegativos(){
    //this.pivotGridDataSource.field("Saldo_Cert", { filterValues: ["-59.24"] });  
    this.pivotGridDataSource.filter(["Saldo_Cert", "<", 0]);
   // this.pivotGridDataSource.field("Saldo_Cert", { filterValues: ["Saldo_Cert", "<", 0] });  
...
*/
    ...
    store: new CustomStore({
      load: function(loadOptions) {      
        console.log(a.mes);         
        return a.httpClient.post("http://rrhh.pvn.gob.pe/api/cap/planilla/proyeccion_dos_cap",{mes:a.mes},
        {headers :header})
        .toPromise()
        .then(result => {          
          return {          
